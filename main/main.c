...
void vTask_Reception() {
  ...
  while (1) {
    ...
    printf(
						"RECEPTION ==> Received request: id: %d, data: %ld\r\n", 
						request_ptr->id, 
						request_ptr->data
					);
		...
		switch(request_ptr->id)
		{
			...
			default:
				printf(
								"RECEPTION ==> No Task handle request id: %d !\r\n",
								request_ptr->id
							);
		}
    ...
    if (xTask_Status == pdPASS) {
      printf(
							"RECEPTION ==> Move request id: %d to queue corresponding!\r\n",
							request_ptr->id
						);
			xTask_Status = pdFAIL;
    }
...
void vTask_1() {
  ...
  while (1) {
    ...
    if (xTask_Status == pdPASS) {
      printf(
							"TASK 1 ==> Handle request: id = %d, data = %ld\r\n",
							request_infor.id,
							request_infor.data
						);
    }
...
void vTask_2() {
  ...
  while (1) {
    ...
    if (xTask_Status == pdPASS) {
      printf(
							"TASK 2 ==> Handle request: id = %d, data = %ld\r\n",
							request_infor.id, 
              request_infor.data
						);
    }
